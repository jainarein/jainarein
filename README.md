# Hi, I'm Arein Jain 👋

🎓 Electronics & Communication Engineering @ JIIT Noida
⚡ AI/ML • Embedded Systems • Edge AI • IoT
🚀 Building intelligent systems combining hardware + AI

---

## 🧠 About Me

* Passionate about AI + Hardware integration (Edge AI, Embedded ML)
* Building real-world systems using satellite data, IoT, and ML
* Interested in efficient AI, hardware acceleration, and real-time systems

---

## 🚀 Projects

### 🌾 GreenKisan – AI Geospatial Intelligence Platform

AI-powered system for crop residue analysis and environmental impact prediction using satellite data
🔧 Improving model accuracy and performance
**Tech:** Python, PyTorch, LSTM, XGBoost, FastAPI, React

---

### 🌿 Green Habitat – Environmental Livability Analysis

Evaluates livability using AQI, satellite NDVI, and urban data
**Tech:** Python, FastAPI, React, Sentinel-2, WAQI

---

### 🌫️ ESP32 Air Quality Monitoring System

Real-time air quality monitoring with embedded web dashboard
**Tech:** ESP32, MQ Sensors, IoT

---

### 📡 Ultrasonic Sensor Radar System

Arduino-based radar system for object detection and visualization
**Tech:** Arduino, Ultrasonic Sensor, Processing

---

### 👁️ Eye Gesture Authentication Lock

ESP32-S3 based authentication system using eye gestures
**Tech:** Embedded Systems

---

### ⚡ PCB Design – Power 3.1 Competition

2nd Position – Designed transmitter & receiver modules
**Tech:** KiCad, Proteus

---

## 🛠️ Skills & Interest Areas

* **Languages:** Python, C/C++
* **AI/ML:** Machine Learning, Deep Learning(Interest Area)
* **Embedded:** Arduino, ESP32, IoT Systems
* **Tools:** FastAPI, React, MATLAB
* **Hardware:** PCB Design (KiCad, Proteus)

---

## 🏆 Achievements

* Top 12 Innovations – DST Govt. of India
* CSIR Innovation Award Finalist (Patent Filed)
* SIH Round 2 Qualifier (National Level)
* 2nd Runner-Up – TesserX Hackathon

---

## 📫 Connect with Me

* LinkedIn: https://www.linkedin.com/in/arein-jain/
* GitHub: https://github.com/jainarein

---

⭐ Always open to collaboration and learning opportunities!
